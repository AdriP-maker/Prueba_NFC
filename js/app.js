/* ══════════════════════════════════════════════════════
  ...
  JSONBin.io para export · jsonstore.io para Sync
══════════════════════════════════════════════════════ */
...
async function btConnect() {
  ...
  try {
    ...
    if (err.name === 'NotFoundError') {
      // Usuario cerró el diálogo sin elegir — no es error real
    } else if (err.name === 'AbortError') {
      showToast('Búsqueda cancelada.', '');
    } else if (err.name === 'TypeError' || (err.message && err.message.includes('optionalServices'))) {
      showToast('Error en lista de servicios. Intenta marcar "Aceptar cualquier dispositivo".', 'error');
      console.error('[BT] requestDevice error:', err.name, err.message);
    } else {
...
function switchTab(name) {
  const tabs = ['write', 'read', 'bluetooth', 'rfid', 'history', 'sync'];
  tabs.forEach(t => {
...
function addToHistory(entry) {
  ...
  saveHistory();
  renderHistory();
  if (State.syncActive && State.syncMode === 'emisor' && State.syncAutoSend) {
    syncSendEntry(entry);
  }
...
function renderHistory() {
  ...
  const typeLabel = {
    ...
    'rfid-read':      '🐄 RFID Animal',
    'sync-received':  '📶 Sync Recibido',
  };
  const typeColor = {
    'write':       'var(--accent)',
    'read':        'var(--success)',
    ...
    'rfid-read':      'var(--success)',
    'sync-received':  'var(--indigo)',
  };
...
const RFID_HM10_SVC  = '0000ffe0-0000-1000-8000-00805f9b34fb';
const RFID_HM10_CHAR = '0000ffe1-0000-1000-8000-00805f9b34fb';
const RFID_NUS_SVC   = '6e400001-b5a3-f393-e0a9-e50e24dcca9e';
const RFID_NUS_TX    = '6e400003-b5a3-f393-e0a9-e50e24dcca9e';

async function rfidConnect() {
  const nameFilter = document.getElementById('rfid-name-filter').value.trim();
  const moduleType = document.getElementById('rfid-module-preset').value;
  rfidState.moduleType = moduleType;

  const requestOpts = { optionalServices: [RFID_HM10_SVC, RFID_NUS_SVC] };
  if (nameFilter) {
    requestOpts.filters = [{ namePrefix: nameFilter }];
  } else {
    requestOpts.acceptAllDevices = true;
  }

  rfidSetStatus('connecting');
  document.getElementById('btn-rfid-connect').disabled = true;

  try {
    const device = await navigator.bluetooth.requestDevice(requestOpts);
    rfidState.device = device;
    device.addEventListener('gattserverdisconnected', rfidOnDisconnected);

    const server = await device.gatt.connect();
    rfidState.server = server;

    let char = null;

    if (moduleType === 'hm10' || moduleType === 'auto') {
      try {
        const svc = await server.getPrimaryService(RFID_HM10_SVC);
        char = await svc.getCharacteristic(RFID_HM10_CHAR);
        rfidState.moduleType = 'hm10';
      } catch {}
    }

    if (!char && (moduleType === 'nus' || moduleType === 'auto')) {
      try {
        const svc = await server.getPrimaryService(RFID_NUS_SVC);
        char = await svc.getCharacteristic(RFID_NUS_TX);
        rfidState.moduleType = 'nus';
      } catch {}
    }

    if (!char) throw new Error('No se encontró característica de datos. Verifica el módulo BT del lector.');

    rfidState.notifyChar = char;
    await rfidStartListen(char);

    const devName = device.name || device.id;
    rfidSetStatus('connected', devName);
    document.getElementById('btn-rfid-connect').style.display = 'none';
    document.getElementById('btn-rfid-disconnect').style.display = '';
    document.getElementById('rfid-live-card').style.display = '';
    document.getElementById('rfid-live-badge').style.display = '';
    showToast(`Lector RFID conectado: ${devName}`, 'success');
    addToHistory({ type: 'bt-connect', data: `RFID Lector: ${devName}` });

  } catch (err) {
    rfidState.device = null;
    rfidState.server = null;
    rfidSetStatus('disconnected');
    document.getElementById('btn-rfid-connect').disabled = false;
...
async function rfidStartListen(char) {
  char.addEventListener('characteristicvaluechanged', rfidOnData);
  await char.startNotifications();
  rfidState.notifying = true;
}

function rfidOnData(event) {
  const chunk = new TextDecoder().decode(event.target.value);
  rfidState.buffer += chunk;
  const lines = rfidState.buffer.split(/\r\n|\r|\n/);
  rfidState.buffer = lines.pop();
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    const parsed = rfidParseId(trimmed);
    rfidAddToSession(parsed);
    rfidUpdateLiveDisplay(parsed);
  }
}

function rfidParseId(raw) {
  const cleaned  = raw.trim();
  const alphanum = cleaned.replace(/[^0-9A-Fa-f\s]/g, '').trim();
  const noSpace  = alphanum.replace(/\s+/g, '');

  // FDX-B decimal: exactly 15 digits
  if (/^\d{15}$/.test(noSpace)) {
    return { raw, normalized: noSpace, format: 'FDX-B', country: noSpace.slice(0, 3), national: noSpace.slice(3) };
  }

  // FDX-B with space: "NNN NNNNNNNNNNNN"
  const spaceMatch = alphanum.match(/^(\d{3})\s+(\d{12})$/);
  if (spaceMatch) {
    return { raw, normalized: spaceMatch[1] + spaceMatch[2], format: 'FDX-B', country: spaceMatch[1], national: spaceMatch[2] };
  }

  // Hex string with alpha chars — try FDX-B decode
  if (/^[0-9A-Fa-f]{12,16}$/.test(noSpace) && /[A-Fa-f]/.test(noSpace)) {
    try {
      const decimal = BigInt('0x' + noSpace).toString();
      if (decimal.length === 15) {
        return { raw, normalized: decimal, format: 'FDX-B (hex)', country: decimal.slice(0, 3), national: decimal.slice(3) };
      }
    } catch {}
    return { raw, normalized: noSpace.toUpperCase(), format: 'HEX', country: null, national: null };
  }

  return { raw, normalized: cleaned, format: 'RAW', country: null, national: null };
}

function rfidAddToSession(parsed) {
  const matched = rfidState.expectedIds.length > 0
    && rfidState.expectedIds.includes(parsed.normalized);

  rfidState.session.unshift({
    id:         rfidState.session.length,
    raw:        parsed.raw,
    normalized: parsed.normalized,
    format:     parsed.format,
    country:    parsed.country  || null,
    national:   parsed.national || null,
    ts:         new Date().toISOString(),
    matched,
    valid:      null,
  });

  addToHistory({ type: 'rfid-read', data: `${parsed.normalized} [${parsed.format}]` });
  rfidRenderSession();
}

function rfidUpdateLiveDisplay(parsed) {
  const display = document.getElementById('rfid-live-display');
  const detail  = document.getElementById('rfid-live-detail');

  display.innerHTML = escapeHTML(parsed.normalized);
  display.classList.remove('rfid-flash');
  void display.offsetWidth;
  display.classList.add('rfid-flash');

  if (parsed.format.startsWith('FDX-B') && parsed.country) {
    const countryName = ISO3166_COUNTRIES[parsed.country] || `País ${parsed.country}`;
    detail.innerHTML = `<strong>${escapeHTML(parsed.format)}</strong> &nbsp;·&nbsp; 🌍 ${escapeHTML(countryName)} (${escapeHTML(parsed.country)}) &nbsp;·&nbsp; ID nacional: <code>${escapeHTML(parsed.national)}</code>`;
  } else {
    detail.innerHTML = `Formato: <strong>${escapeHTML(parsed.format)}</strong>`;
  }
...
function syncSetMode(mode) {
  State.syncMode = mode;
  document.getElementById('btn-sync-emisor').classList.toggle('sync-mode-btn--active', mode === 'emisor');
  document.getElementById('btn-sync-receptor').classList.toggle('sync-mode-btn--active', mode === 'receptor');
  document.getElementById('sync-generate-section').style.display = mode === 'emisor'   ? '' : 'none';
  document.getElementById('sync-join-section').style.display     = mode === 'receptor' ? '' : 'none';
  ...
  syncUpdateStatusBar('idle');
  if (State.syncActive) syncDisconnect();
}
...
function syncRenderReading(reading) {
  const feed  = document.getElementById('sync-live-feed');
  const empty = feed.querySelector('.empty-state');
  if (empty) empty.remove();

  const typeMap = { 'rfid-read': 'rfid', 'read': 'nfc', 'simulated': 'nfc', 'write': 'nfc' };
  const cls     = typeMap[reading.type] || '';
  const ts      = new Date(reading.ts).toLocaleString('es-PA');
  const item    = document.createElement('div');
  item.className = `sync-feed-item${cls ? ` sync-feed-item--${cls}` : ''}`;
  item.innerHTML = `
    <div class="sync-feed-item-time">${escapeHTML(ts)}</div>
    <div class="sync-feed-item-data">${escapeHTML(reading.data || '—')}</div>
    <div class="sync-feed-item-type">${escapeHTML(reading.type || 'lectura')}</div>`;
  feed.prepend(item);

  const countEl = document.getElementById('sync-feed-count');
  countEl.style.display = '';
  countEl.textContent   = parseInt(countEl.textContent || '0') + 1;

  addToHistory({ type: 'sync-received', data: reading.data, extra: reading.type });
...
function syncDisconnect() {
  ...
  if (State.syncMode === 'emisor') {
    ...
    document.getElementById('sync-join-section').style.display = '';
  }
  syncUpdateStatusBar('idle');
  syncUpdateSendButtons();
...
function syncClearFeed() {
  const feed = document.getElementById('sync-live-feed');
  feed.innerHTML = '<p class="empty-state">Esperando lecturas del Emisor…</p>';
  const countEl = document.getElementById('sync-feed-count');
...
function rfidRenderSession() {
  const wrap    = document.getElementById('rfid-session-table-wrap');
  const session = rfidState.session;
  const total   = session.length;
  const matched = session.filter(e => e.matched).length;

  document.getElementById('rfid-session-counts').innerHTML = `
    <span class="rfid-count rfid-count--total">${total} leídos</span>
    <span class="rfid-count rfid-count--match">${matched} esperados</span>
    <span class="rfid-count rfid-count--new">${total - matched} nuevos</span>`;

  if (!session.length) {
    wrap.innerHTML = '<p class="empty-state">Sin lecturas en esta sesión.</p>';
    return;
  }

  const rows = session.map((e, i) => {
    const fmtKey   = e.format.startsWith('FDX-B') ? 'fdxb' : e.format === 'HEX' ? 'hex' : '';
    const fmtBadge = `<span class="rfid-badge-format rfid-badge-format--${fmtKey}">${escapeHTML(e.format)}</span>`;
    const matchCell = rfidState.expectedIds.length
      ? (e.matched ? '<span class="rfid-match-yes">✓ Sí</span>' : '<span class="rfid-match-no">— No</span>')
      : '<span style="color:var(--text-muted)">—</span>';
    const resultCell = e.valid === true
      ? '<span class="rfid-result-valid">✓ Válido</span>'
      : e.valid === false
        ? '<span class="rfid-result-invalid">✗ Inválido</span>'
        : `<button class="rfid-mark-btn" onclick="rfidMarkRow(${i},true)" title="Marcar válido">✓</button><button class="rfid-mark-btn" onclick="rfidMarkRow(${i},false)" title="Marcar inválido">✗</button>`;
    const rowClass = e.valid === true ? 'rfid-row--valid' : e.valid === false ? 'rfid-row--invalid' : (e.matched ? 'rfid-row--matched' : 'rfid-row--unmatched');
    const ts = new Date(e.ts).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

    return `<tr class="${rowClass}">
      <td style="color:var(--text-secondary);font-size:0.72rem;white-space:nowrap">${ts}</td>
      <td class="rfid-id-cell">${escapeHTML(e.normalized)}</td>
      <td>${fmtBadge}</td>
      <td>${matchCell}</td>
      <td>${resultCell}</td>
    </tr>`;
  ...
  wrap.innerHTML = `<table class="rfid-table">
    <thead><tr>
      <th>Hora</th><th>ID Normalizado</th><th>Formato</th><th>¿Esperado?</th><th>Resultado</th>
    </tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}

function rfidMarkRow(index, valid) {
  if (rfidState.session[index]) {
    rfidState.session[index].valid = valid;
    rfidRenderSession();
  }
}

function rfidLoadExpected() {
  const text = document.getElementById('rfid-expected-input').value.trim();
  if (!text) { showToast('Ingresa IDs en el campo de texto.', 'error'); return; }
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  rfidState.expectedIds = lines.map(l => rfidParseId(l).normalized);
  const countEl = document.getElementById('rfid-expected-count');
  countEl.textContent = `✓ ${rfidState.expectedIds.length} IDs cargados para validación`;
  countEl.style.display = '';
  showToast(`${rfidState.expectedIds.length} IDs esperados cargados.`, 'success');
  rfidRenderSession();
}

function rfidClearExpected() {
  rfidState.expectedIds = [];
  document.getElementById('rfid-expected-input').value = '';
  document.getElementById('rfid-expected-count').style.display = 'none';
  rfidRenderSession();
  showToast('Lista esperada limpiada.');
}

function rfidSimulate() {
  const val = document.getElementById('rfid-sim-input').value.trim();
  if (!val) { showToast('Ingresa un ID para simular.', 'error'); return; }
  const parsed = rfidParseId(val);
  rfidAddToSession(parsed);
  rfidUpdateLiveDisplay(parsed);
  document.getElementById('rfid-live-card').style.display = '';
  showToast('Lectura RFID simulada.', 'success');
}

function rfidClearSession() {
  if (!rfidState.session.length) return;
  if (!confirm('¿Limpiar todas las lecturas de la sesión actual?')) return;
  rfidState.session = [];
  rfidRenderSession();
  showToast('Sesión limpiada.');
}

function rfidExportCSV() {
  if (!rfidState.session.length) { showToast('No hay lecturas para exportar.', 'error'); return; }
  const header = 'Timestamp,ID_Raw,ID_Normalizado,Formato,Pais_Codigo,Codigo_Nacional,En_Lista_Esperada,Resultado\n';
  const rows = rfidState.session.map(e => [
    e.ts,
    `"${e.raw.replace(/"/g, '""')}"`,
    e.normalized,
    e.format,
    e.country  || '',
    e.national || '',
    rfidState.expectedIds.length ? (e.matched ? 'Si' : 'No') : '',
    e.valid === true ? 'Valido' : e.valid === false ? 'Invalido' : 'Pendiente',
  ].join(',')).join('\n');
  const ts = new Date().toISOString().slice(0, 16).replace('T', '_').replace(':', '-');
  rfidDownload(new Blob(['﻿' + header + rows], { type: 'text/csv;charset=utf-8;' }), `rfid_sesion_${ts}.csv`);
  showToast('CSV descargado.', 'success');
}

function rfidExportJSON() {
  if (!rfidState.session.length) { showToast('No hay lecturas para exportar.', 'error'); return; }
  const data = {
    app:      'NFC-BT-Toolkit · RFID Ganadero',
    exported: new Date().toISOString(),
    device:   rfidState.device?.name || 'Simulado',
    module:   rfidState.moduleType,
    expected: rfidState.expectedIds,
    session:  rfidState.session,
  };
  const ts = new Date().toISOString().slice(0, 16).replace('T', '_').replace(':', '-');
  rfidDownload(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }), `rfid_sesion_${ts}.json`);
  showToast('JSON descargado.', 'success');
}

function rfidDownload(blob, filename) {
...
async function rfidDisconnect() {
  if (rfidState.notifyChar && rfidState.notifying) {
    try { await rfidState.notifyChar.stopNotifications(); } catch {}
    rfidState.notifyChar.removeEventListener('characteristicvaluechanged', rfidOnData);
  }
  if (rfidState.device?.gatt?.connected) {
    try { rfidState.device.gatt.disconnect(); } catch {}
  }
  rfidOnDisconnected();
}

function rfidOnDisconnected() {
  rfidState.device     = null;
  rfidState.server     = null;
  rfidState.notifyChar = null;
  rfidState.notifying  = false;
  rfidState.buffer     = '';
  rfidSetStatus('disconnected');
  document.getElementById('btn-rfid-connect').style.display = '';
  document.getElementById('btn-rfid-disconnect').style.display = 'none';
  document.getElementById('rfid-live-badge').style.display = 'none';
  showToast('Lector RFID desconectado.');
}

function rfidSetStatus(state, deviceName) {
  const bar    = document.getElementById('rfid-status');
  const textEl = document.getElementById('rfid-status-text');
  bar.className = `rfid-status-bar rfid-status--${state}`;
  textEl.textContent = state === 'connected' ? `Conectado: ${deviceName}`
    : state === 'connecting' ? 'Conectando…'
    : 'Desconectado';
}

const ISO3166_COUNTRIES = {
  '032': 'Argentina', '076': 'Brasil',     '124': 'Canadá',
  '152': 'Chile',     '156': 'China',      '170': 'Colombia',
  '188': 'Costa Rica','191': 'Croacia',    '203': 'Chequia',
  '208': 'Dinamarca', '214': 'Rep. Dom.',  '218': 'Ecuador',
  '246': 'Finlandia', '250': 'Francia',    '276': 'Alemania',
  '300': 'Grecia',    '320': 'Guatemala',  '340': 'Honduras',
  '348': 'Hungría',   '372': 'Irlanda',    '380': 'Italia',
  '392': 'Japón',     '484': 'México',     '528': 'Países Bajos',
  '558': 'Nicaragua', '578': 'Noruega',    '591': 'Panamá',
  '600': 'Paraguay',  '604': 'Perú',       '616': 'Polonia',
  '620': 'Portugal',  '630': 'Puerto Rico','642': 'Rumania',
  '703': 'Eslovaquia','705': 'Eslovenia',  '724': 'España',
  '752': 'Suecia',    '756': 'Suiza',      '792': 'Turquía',
  '826': 'R. Unido',  '840': 'EE.UU.',     '858': 'Uruguay',
  '862': 'Venezuela',
};
